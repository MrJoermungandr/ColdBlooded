...
func submit_pb(run:LevelRun):
	...
	request.request(API_URL+"/times/submit/"+run.level_name,auth_header,HTTPClient.METHOD_POST,JSON.stringify({"level_time":run.level_time,"is_coin_percent":run.is_coin_percent}))
